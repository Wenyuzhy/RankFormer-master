...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        im1 = im1.cuda()
        im2 = im2.cuda()
        output_rgb, coarse_rgb = model(im1)  
        gt_rgb = im2
        loss_0 = L1_loss(coarse_rgb, gt_rgb) + D_loss(coarse_rgb, gt_rgb) + E_loss(coarse_rgb, gt_rgb) + opt.P_weight * P_loss(coarse_rgb, gt_rgb)[0]
        loss_1 = L1_loss(output_rgb, gt_rgb) + D_loss(output_rgb, gt_rgb) + E_loss(output_rgb, gt_rgb) + opt.P_weight * P_loss(output_rgb, gt_rgb)[0]
        loss = loss_1 + 0.1*loss_0
        iter += 1
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        pic_cnt += 1
        pic_last_10 += 1
        train_loss = loss_last_10/pic_last_10
        if iter == train_len:
    ...
    return loss_print, pic_cnt, train_loss
                

def checkpoint(epoch):
    if not os.path.exists(opt.val_folder +'/weights_1'):          
        os.mkdir(opt.val_folder +'/weights_1') 
    if not os.path.exists(opt.val_folder +'/weights_1/train'):          
        os.mkdir(opt.val_folder +'/weights_1/train')  
    model_out_path = opt.val_folder + "/weights_1/train/epoch_{}.pth".format(epoch)
...
def build_model():
    ...
    if opt.start_epoch > 0:
        pth = opt.val_folder + f"/weights/train/epoch_{opt.start_epoch}.pth"
...
def init_loss():
    ...
    L1_loss = Loss_MSE(loss_weight=L1_weight).cuda()
...
if __name__ == '__main__':  
    ...
    # L1_loss,E_loss,D_loss = init_loss()
    ...
    if not os.path.exists(opt.val_folder):
        os.makedirs(opt.val_folder)
    log_dir = os.path.join(opt.val_folder, 'train.log')
    logger = initialize_logger(log_dir)

    record_psnr = 18.0
    epoch = start_epoch
        
    while epoch < opt.nEpochs + start_epoch +1 :
        epoch_loss, pic_num, train_loss = train(epoch)
        scheduler.step()
        epoch = epoch +1 
        if epoch % opt.snapshots == 0:
            ...
            print(f"{ssim}")
            print(f"{lpips}")
            if avg_psnr > record_psnr or epoch % 10 == 0:
                print(f'Saving model...')
                model_out_path = checkpoint(epoch)
                if avg_psnr > record_psnr:
                    record_psnr = avg_psnr
            logger.info("Epoch[%06d], Train Loss: %.9f, Test PSNR: %.4f, Test SSIM: %.4f, Test LPIPS: %.4f " % (epoch, train_loss, avg_psnr, avg_ssim, avg_lpips))
        torch.cuda.empty_cache()
        
    # if not os.path.exists(opt.val_folder + '/metrics.md'):          
    #             os.mkdir(opt.val_folder + '/metrics.md') 
    with open(opt.val_folder + '/metrics.md', "w") as f:
